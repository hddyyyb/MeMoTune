...
from src.loralayer import Conv2d, LoraLayer, dispatch_default
...
class LoraModel(BaseTuner):
    ...
    **Attributes**:
    ...
    def _create_and_replace(
        ...
        if isinstance(target, LoraLayer) and not isinstance(target, AdaLoraLayer): # never come in (for init+ finetune)
            target.update_layer(
                adapter_name,
                r,
                lora_alpha=alpha,
        ...
        else:  # always come in for init
            # init: init LoraLayer+LoraLayer.update_layer
            # finetune: init LoraLayer
            new_module = self._create_new_module(lora_config, adapter_name, target, **kwargs)  
            if adapter_name not in self.active_adapters:  # never come in (for init+ finetune)
    ...
    def _create_new_module(lora_config, adapter_name, target, **kwargs):
        ...
        if is_bnb_available():
            from src.bnb import dispatch_bnb_8bit

            dispatchers.append(dispatch_bnb_8bit)

        if is_bnb_4bit_available():
            from src.bnb import dispatch_bnb_4bit
