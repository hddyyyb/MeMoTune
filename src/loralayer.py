...
from src._sdp_loralayer import (
    BinaryQuantize,
    BinaryQuantize_IRNet,
    BinaryQuantize_RBNN,
    BinaryQuantize_a,
    BinaryQuantize_a_RBNN,
)
...
class BinarizeConv2dSDP(nn.Module):
    ...
    def __init__(self, K, scale, in_chn, out_chn, kernel_size=3, 
        ...
        self.number_of_weights = in_chn * out_chn * kernel_size * kernel_size  # 1
        ...
        self.M = nn.Parameter(torch.zeros((self.shape_sum_w)), requires_grad=True)  # 1
        self.Z = nn.Parameter(torch.zeros((self.shape_w)), requires_grad=True)  #  1
    ...
    def forward(self, input):    
        ...
        w = SDP().apply(m, z, self.sample, torch.tensor(self.scale), torch.tensor(self.K))
...
class LoraLayer(BaseTunerLayer):
    ...
    adapter_layer_names = ("lora_embedding_A", "lora_embedding_B")
    other_param_names = ("K", "r", "lora_alpha", "scaling", "lora_dropout")
    ...
    def __init__(self, base_layer: nn.Module, **kwargs) -> None:
        ...
        self.K = {} 
        self.scale = {} 
        self.number_of_weightsA = {}  
        self.number_of_weightsB = {}
        self.shape_wA = {}  
        self.shape_wB = {}  
        self.shape_sum_wB = {}  
        self.shape_sum_wA = {}  

        self.lora_A_M = nn.ParameterDict({})  
        self.lora_B_M = nn.ParameterDict({}) 
        self.lora_A_Z = nn.ParameterDict({}) 
        self.lora_B_Z = nn.ParameterDict({}) 
        self.sampleA = nn.ParameterDict({}) 
        self.sampleB = nn.ParameterDict({}) 
    ...
    def update_layer(
        self, K,scale, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights, use_rslora, use_dora: bool = False
    ):  
        ...
        self.shape_wA = (K, r, self.in_features) 
        self.shape_sum_wA = (r, self.in_features)
        self.lora_A_M[adapter_name] = nn.Parameter(torch.zeros((self.shape_sum_wA)), requires_grad=True) 
        self.lora_A_Z[adapter_name] = nn.Parameter(torch.zeros((self.shape_wA)), requires_grad=True)
        self.sampleA[adapter_name] = nn.Parameter(torch.zeros((1, K)), requires_grad=False)
        
        torch.nn.init.xavier_normal_(self.sampleA[adapter_name])

        self.number_of_weightsB = r * self.out_features  
        self.shape_wB = (K,  self.out_features, r) 
        self.shape_sum_wB = (self.out_features, r)  
        self.lora_B_M[adapter_name] = nn.Parameter(torch.zeros((self.shape_sum_wB)), requires_grad=True) 
        self.lora_B_Z[adapter_name] = nn.Parameter(torch.zeros((self.shape_wB)), requires_grad=True)
        self.sampleB[adapter_name] = nn.Parameter(torch.zeros((1, K)), requires_grad=False)  
        torch.nn.init.xavier_normal_(self.sampleB[adapter_name])
    ...
    def reset_lora_parameters(self, adapter_name, init_lora_weights):
        ...
        if adapter_name in self.lora_A_M.keys():
            if init_lora_weights is True:
                nn.init.kaiming_uniform_(self.lora_A_M[adapter_name], a=math.sqrt(5))
            elif init_lora_weights.lower() == "gaussian":
                nn.init.normal_(self.lora_A_M[adapter_name], std=1 / self.r[adapter_name])
            else:
                raise ValueError(f"Unknown initialization {init_lora_weights=}")
            nn.init.zeros_(self.lora_B_M[adapter_name])
    ...
    def pissa_init(self, adapter_name, init_lora_weights):
        ...
        self.lora_A_M[adapter_name] = lora_A
        self.lora_B_M[adapter_name] = lora_B
        weight = weight.data - self.scaling[adapter_name] * lora_B @ lora_A
        weight = weight.to(dtype)
        self.get_base_layer().weight.data = weight

    def loftq_init(self, adapter_name):
        from src.loftq_utils import loftq_init  # need to be change
    ...
    def dora_init(self, adapter_name: str) -> None:
        lora_A = self.lora_A_M[adapter_name]
        lora_B = self.lora_B_M[adapter_name]
    ...
    def scale_layer(self, scale: float) -> None:
        ...
        for active_adapter in self.active_adapters:
            if active_adapter not in self.lora_A_M.keys():
                continue
    ...
    def _mixed_batch_forward(
        ...
        for i, active_adapter in enumerate(unique_adapters):
            ...
            lora_A = self.lora_A_M[active_adapter]
            lora_B = self.lora_B_M[active_adapter]
...
class Linear(nn.Module, LoraLayer):
    ...
    def merge(self, safe_merge: bool = False, adapter_names: Optional[list[str]] = None) -> None:
        ...
        for active_adapter in adapter_names:
            if active_adapter in self.lora_A_M.keys():
                base_layer = self.get_base_layer()
                if safe_merge:
    ...
    def get_delta_weight(self, adapter) -> torch.Tensor:
        """
        Compute the delta weight for the given adapter.  计算给定适配器的增量权重。
        ...
        weight_A = self.lora_A_M[adapter]
        weight_B = self.lora_B_M[adapter]
        ...
        if cast_to_fp32:
            ...
            self.lora_A_M[adapter] = weight_A.to(dtype)
            self.lora_B_M[adapter] = weight_B.to(dtype)
    ...
    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                wA = SDP().apply(mA, zA, self.sampleA[active_adapter], torch.tensor(self.scale), torch.tensor(self.K))
                ...
                wB = SDP().apply(mB, zB, self.sampleB[active_adapter], torch.tensor(self.scale), torch.tensor(self.K))
                real_weightsB = wB.view(self.shape_sum_wB)

                dropout = self.lora_dropout[active_adapter]
                scaling = self.scaling[active_adapter]
                requires_conversion = not torch.is_autocast_enabled()
                if requires_conversion:  # go in
                    expected_dtype = result.dtype
                    x = x.to(real_weightsA.dtype)

                if not self.use_dora[active_adapter]:  # go in
                    outputtmp = F.linear(dropout(x), real_weightsA)
                    output = F.linear(outputtmp, real_weightsB) * scaling
                else:
                    print(' _apply_dora is not implemented ')
                    sys.exit(1)
                if requires_conversion:  # go in
...
def dispatch_default(
    ...
    **kwargs,
    ...
    if isinstance(target_base_layer, torch.nn.Embedding):
        ...
        kwargs.update(lora_config.loftq_config)
        # ---target:Linear(in_features=3072, out_features=768, bias=True), 
        # adapter_name:default,kwargs:{'r': 16, 'lora_alpha': 16, 'lora_dropout': 0.1, 
        # 'fan_in_fan_out': False, 'init_lora_weights': 'loftq', 'use_rslora': False, 
        # 'use_dora': False, 'loaded_in_8bit': False, 'loaded_in_4bit': False, 
        # 'loftq_bits': 4, 'loftq_iter': 5}
