...
from src.loralayer import LoraLayer, SDP
import sys
...
if is_bnb_available():
    ...
    def dispatch_bnb_8bit(target: torch.nn.Module, adapter_name: str, **kwargs):
        ...
        if loaded_in_8bit and isinstance(target_base_layer, bnb.nn.Linear8bitLt):  # not in!
    ...
    class Linear4bit(torch.nn.Module, LoraLayer):
        ...
        def forward(self, x: torch.Tensor, *args, **kwargs) -> torch.Tensor:
            ...
            if self.disable_adapters:
                ...
                # GO IN THIS, it add A*B*x with original w*x 
                result = self.base_layer(x, *args, **kwargs)
                result = result.clone()
