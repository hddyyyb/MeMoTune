...
class CustomSeq2SeqTrainer(Seq2SeqTrainer):
    def evaluate(self, eval_dataset=None, ignore_keys=None, metric_key_prefix="eval"):
        self.model.eval()
        L = 3  # Number of forward passes to average over

        total_loss = 0
        total_output = 0
        for _ in range(L):
            # Perform one forward pass with no gradient calculation
            with torch.no_grad():
                output = self.model(**self.tokenizer(eval_dataset['input_text'], return_tensors='pt').to(self.model.device))
                loss = self.compute_loss(self.model, output, eval_dataset['labels'])
                total_loss += loss
                total_output += output

        # Average the loss and outputs
        total_loss /= L
        total_output /= L

        # Return the metrics as usual (modify as needed)
        metrics = {'eval_loss': total_loss.item(), 'eval_output': total_output}
