...
from src.loramodel import LoraModel
from src.tuners_utils import BaseTuner, BaseTunerLayer
    ...
    **Attributes**:
    ...
    def __init__(self, model: PreTrainedModel, peft_config: PeftConfig, adapter_name: str = "default") -> None:
        ...
        if self._is_prompt_learning:
            ...
            self.base_model = cls(model, {adapter_name: peft_config}, adapter_name)  # init 
    ...
    def from_pretrained(
        ...
        **kwargs: Any,
        ...
        # load the config
        if config is None:  # go in
            tempc = PeftConfig._get_peft_type(
                    model_id,
            ...
            config = PEFT_TYPE_TO_CONFIG_MAPPING[tempc].from_pretrained(model_id, **kwargs)  # rank =16 is setting
        ...
        if hasattr(model, "hf_device_map"):  # in
    ...
    def forward(
        ...
        **kwargs,
        ...
        if not peft_config.is_prompt_learning:
            ...
            # self.base_model: LoraModel(  (model): LlamaForCausalLM(    (model): LlamaModel(
            with self._enable_peft_forward_hooks(**kwargs):
                kwargs = {k: v for k, v in kwargs.items() if k not in self.special_peft_forward_args}
                return self.base_model(
                    input_ids=input_ids,
