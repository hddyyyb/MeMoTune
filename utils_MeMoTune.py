...
class LinearQuantLoRA(nn.Module):
    def __init__(self, in_feature, out_feature, reduced_rank, has_bias=True, args=None):
        ...
        if self.has_svd_adapter:
            ...
            self.shape_wright = (MeMoTune_K, reduced_rank, in_feature)
            ...
            self.sampleright  = nn.Parameter(torch.zeros((1, MeMoTune_K)), requires_grad=False)
            torch.nn.init.xavier_normal_(self.sampleright)
            self.number_of_weightsleft = reduced_rank * out_feature
            self.shape_wleft = (MeMoTune_K,  out_feature, reduced_rank)
            ...
            self.sampleleft = nn.Parameter(torch.zeros((1, MeMoTune_K)), requires_grad=False)
    ...
    def forward(self, x):
        ...
        if self.has_svd_adapter:
            ...
            wright = MeMoTune().apply(mright, zright, self.sampleright, torch.tensor(self.scale), torch.tensor(self.K))
        ...
        if self.has_svd_adapter:
            ...
            wleft = MeMoTune().apply(mleft, zleft, self.sampleleft, torch.tensor(self.scale), torch.tensor(self.K))
    ...
    def initialize_weight(self, quant_weight, left_weight, right_weight, sparse_weight=None, bias=None):
        ...
        if self.has_svd_adapter:
            self.left_M = nn.Parameter(left_weight, requires_grad=True)
            self.right_M = nn.Parameter(right_weight, requires_grad=True)
